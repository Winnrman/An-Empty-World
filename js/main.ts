import * as achievements from "./achievements"; // this may look unnecessary but it's needed to load the file to start the interval. We might want to make this explicit by calling the start interval in this file
import * as activities from "./activities";
import * as crafting from "./crafting";
import player, { saveData, resetData } from "./player";
import * as store from "./store";
import * as questing from "./questing";
import { loadOptionsFromOwnedEquipment } from "./maintenance";
...
function checkLevelUnlocks() {
    ...
    if (player.level >= 5) {
        for (var i = 0; i < 4; i++) {
            (document.getElementsByClassName("level5Crafting")  as HTMLCollectionOf<HTMLElement>)[i].style.visibility = "visible";
            (document.getElementsByClassName("level5Crafting")  as HTMLCollectionOf<HTMLElement>)[i].style.display = "block";
        }
    }
    if (player.level > 7) {
...
function checkDarkMode() {
    ...
    if (hours >= 15 || hours < 8) {
        ...
        var headers4 = document.getElementsByTagName("select");
        for (let i = 0; i < headers4.length; i++) {
            Array.prototype.forEach.call(headers4, function (header4) {
