...
setInterval(function () {
    ...
    document.getElementById("lvl").innerHTML = player.level.toString();
...
function checkAttackStatus() {
    if (isAttacking == true) {
        ...
        setInterval(function () {
            if (parseFloat(document.getElementById("healthValue").innerHTML) > 0 && fled == false) {
                doEnemyAttack();
            }
        }
            , (1000 / enemySpeed));
...
export function doAttack() {
    const opponent = enemy_dictionary[(<HTMLInputElement>document.getElementById("opponentSelector")).value];
    var enemyHealth = opponent.health; // <-- this works in browser but not in VSCode
    var enemyDefense = opponent.defense;
    var enemyName = opponent.name;
    var enemyGold = opponent.gold;
    var enemyExperience = opponent.defeatExperience;
    ...
    refresh = setInterval(function () {
        if (newEnemyHealth > 0 && fled == false) {
            ...
            document.getElementById("healthValue").innerHTML = Math.max(0, newEnemyHealth).toString();
...
export function doFlee() {
    ...
    document.getElementById("playerHealthValue").innerHTML = player.playerHealth.toString();
    ...
    document.getElementById("playerSpeedValue").innerHTML = player.playerSpeed.toString();

}

function doEnemyAttack() {
    var enemyAttack = enemy_dictionary[(<HTMLInputElement>document.getElementById("opponentSelector")).value].attack;
    ...
    document.getElementById("playerHealthValue").innerHTML = Math.max(0, newPlayerHealth).toString();
